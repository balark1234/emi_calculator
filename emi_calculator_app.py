#!/usr/bin/env python3
"""
EMI Calculator Web App - Streamlit (Full Version with All Features)
"""
...
def generate_schedule(principal, annual_rate, tenure_months, loan_type="normal", monthly_emi=None, extras=None, start_date=None):
    if extras is None:
        extras = []
...
def make_display_schedule(df):
    if df.empty:
        return df
    display_df = df.copy()
    for col in ["Payment (₹)", "Interest (₹)", "Principal (₹)", "Extra (₹)", "Outstanding Principal (₹)", "Cumulative Interest (₹)"]:
...
if STREAMLIT_AVAILABLE:
    ...
    schedule_df, _ = generate_schedule(principal, annual_rate, tenure_months, loan_type_key, base_monthly, [])
    ...
    if not schedule_df.empty:
        ...
        if PLOTLY_AVAILABLE:
            ...
            fig.add_trace(go.Scatter(x=yg['Year'], y=yg['Balance'], name='Outstanding Balance', mode='lines+markers', line=dict(color='#8e44ad', width=3), marker=dict(size=7)), secondary_y=True)
